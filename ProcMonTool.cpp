...
class DLLInfo 
{
   private:
	   ...
	   MODULEENTRY32 me32;
	   HANDLE hProcessSnap; 
	   static int iDLLRefCount;
   public:
	   DLLInfo(DWORD); 
	   BOOL DependentDLLDisplay(); 
	   static void DisplayReferenceCount();
};

int DLLInfo::iDLLRefCount=0;
void DLLInfo::DisplayReferenceCount()
{
	cout<<endl<<"Reference count of "<<DLLName<<" is "<<iDLLRefCount<<endl;
	iDLLRefCount=0;
	DLLName="";
}
...
{ 
	...
	{ 
	 ...
	 do
	 {
		  ...
		  cout<<arr<<endl;
		  if((_stricmp(DLLName,"")!=0)&&(_stricmp(arr,DLLName)==0))
		  {
				iDLLRefCount++;
		  }
...
{ 
    ...
    char FileName[50], arr[512]; 
	 ...
	 sprintf_s(FileName,"C://MarvellousLog//%02d_%02d_%02d_%s.txt",lt.wHour,lt.wMinute,lt.wDay,month[lt.wMonth-1]); 
...
{ 
	char FileName[50]; 
	char* montharr[]={"JAN","FEB","MAR","APR","MAY","JUN","JUL","AUG","SEP","OCT","NO V","DEC"}; 
	int ret = 0, count = 0; 
	LOGFILE fobj; 
	FILE *fp; 
...
void ManPage(char option[])
{
	char *filename=new char[84];

	if(_stricmp(option,"search")==0)
	{
		filename="search.txt";
	}
	else if(_stricmp(option,"kill")==0)
	{
		filename="kill.txt";
	}
	else
...
int main(int argc, char* argv[]) 
{
	...
	while(1) 
	{
		...
		if(count == 1)
	   {
		...
		{
			if((_stricmp(command[0],"ps")==0)&&(_stricmp(command[1],"-d")==0)&&(_stricmp(command[2],"-c")==0))
			{
				DLLName=command[3];
				ProcessInfo *obj=new ProcessInfo();
				obj->ProcessDisplay("-d");
				system("cls");
				DLLInfo::DisplayReferenceCount();
			}
		}
		else 
		{ 
